    ...
    <main class="flex-grow">
      <SectionWrapper>
        <div v-if="post" class="max-w-4xl mx-auto">
          ...
          <RouterLink
            to="/blog"
            class="inline-flex items-center text-gray-700 dark:text-gray-300 hover:text-brand-yellow mb-8 transition-colors group"
          >
          ...
          <article v-motion-slide-up>
            ...
            <div class="mb-6">
              ...
              <h1 class="text-4xl md:text-5xl font-bold mb-6 text-gray-900 dark:text-white">{{ post.title }}</h1>
            </div>
            ...
            <!-- Share Buttons -->
            <div class="mb-12 pt-8 border-t border-gray-300 dark:border-gray-700">
              <h3 class="text-lg font-semibold mb-4 text-gray-900 dark:text-white">{{ $t('blog.share') }}</h3>
              <div class="flex flex-wrap gap-3">
                <button
                  @click="shareOnSocial('instagram')"
                  class="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-purple-600 via-pink-600 to-orange-500 hover:from-purple-700 hover:via-pink-700 hover:to-orange-600 text-white transition-colors duration-200"
                >
                  ...
                  <span class="text-sm font-medium">{{ $t('blog.social.instagram') }}</span>
                </button>
                <button
...
const shareOnSocial = async (platform: string) => {
  ...
  if (platform === 'telegram') {
    ...
    if (isMobile) {
      // Для iOS используем веб версию Telegram, которая правильно откроет приложение
      if (isIOS) {
        // На iOS веб версия Telegram автоматически предложит открыть приложение
        // и правильно передаст параметры для шаринга
        window.open(telegramUrl, '_blank')
        return
      }
      
      // Для Android используем Web Share API если доступен
      if (navigator.share) {
        try {
      ...
      // Для Android пробуем открыть Telegram приложение напрямую
      const telegramAppUrl = `tg://share?url=${encodeURIComponent(url)}&text=${encodeURIComponent(title + '\n\n' + text)}`
      window.location.href = telegramAppUrl
      
      // Fallback на веб версию если приложение не открылось
      setTimeout(() => {
      ...
      // Для десктопа открываем веб версию
      window.open(telegramUrl, '_blank', 'width=600,height=500')
    }
  } else if (platform === 'instagram') {
    const isMobile = /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)
    
    // Для мобильных устройств используем Web Share API если доступен
    ...
    const copyToClipboard = async (text: string): Promise<boolean> => {
      ...
      // Fallback метод для старых браузеров (работает лучше на мобильных)
      try {
        ...
        // Для iOS нужно сделать элемент видимым и выделить
        if (isMobile) {
    ...
    if (!copied && !isMobile) {
      // Только на десктопе показываем уведомление если не удалось скопировать
      alert('Не удалось скопировать ссылку автоматически. Ссылка: ' + url)
    }
    
    // Открываем Instagram
    if (isMobile) {
      // Для мобильных устройств открываем приложение Instagram Direct
      // Используем небольшую задержку чтобы копирование успело завершиться
      setTimeout(() => {
        window.location.href = 'instagram://direct'
      }, 300)
    } else {
      // Для десктопа открываем веб версию Instagram Direct
      setTimeout(() => {
